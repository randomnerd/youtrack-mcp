 ...
 */
export function mapIssueToAIReadableText(
  issue: Issue,
...
Project: ${issueData.project?.name || 'Unknown'} (${
    issueData.project?.shortName || 'N/A'
  })
`;
  ...
  result += `Created: ${formatDate(createdDate)}\n`;
  result += `Updated: ${formatDate(updatedDate)}\n`;
  result += `${resolvedDate ? `Resolved: ${formatDate(resolvedDate)}\n` : ''}`;
  ...
  if (issueData.reporter) {
    ...
    result += `Reporter: ${reporter.fullName || reporter.login || 'Unknown'} (${
  ...
  if (issueData.assignee || issueData.assignees) {
    const assignees =
      issueData.assignees || (issueData.assignee ? [issueData.assignee] : []);
    if (assignees.length > 0) {
      result += `Assignee(s): ${assignees
        .map((a: any) => `${a.fullName || a.login || 'Unknown'}`)
        .join(', ')}\n`;
  ...
  if (issueData.updater) {
    result += `Last Updated By: ${
      issueData.updater.fullName || issueData.updater.login || 'Unknown'
    }\n`;
    ...
    for (const field of fields) {
      ...
      if (value === null || value === undefined) {
        ...
        if (Array.isArray(value)) {
          value = value
            .map((v: any) => v.name || v.fullName || v.login || v)
            .join(', ');
        } else {
  ...
  if (issueData.dueDate) {
    ...
    result += `Due Date: ${formatDate(dueDate)}\n`;
  }
  ...
  const storyPointsField = fields.find(
    (f) => f.name === 'Story points' || f.name === 'Story Points'
  );
  ...
  if (issueData.watchers) {
    if (Array.isArray(issueData.watchers)) {
      result += `\nWatchers: ${issueData.watchers.length} (${issueData.watchers
        .map((w: any) => w.fullName || w.login)
        .join(', ')})\n`;
  ...
  // Format linked issues if available
  if (issueData.links && issueData.links.length > 0) {
    ...
    for (const link of issueData.links) {
      // Use all possible link type properties, prioritizing direction for inward/outward links
      const linkType =
        link.direction ||
        link.linkType?.sourceToTarget ||
        link.linkType?.name ||
        'relates to';
      const linkedIssues = link.issues || link.trimmedIssues || [];

      if (!linksByType[linkType]) {
        linksByType[linkType] = [];
      }

      linkedIssues.forEach((linkedIssue: any) => {
        const id = linkedIssue.idReadable || linkedIssue.id || 'unknown';
        const summary = linkedIssue.summary || 'No summary';
        linksByType[linkType].push(`${id} (${summary})`);
      });
    }

    for (const [type, issues] of Object.entries(linksByType)) {
      result += `  ${type}: ${issues.join(', ')}\n`;
    }
  }
  ...
  if (issueData.attachments && issueData.attachments.length > 0) {
    ...
    for (const att of issueData.attachments) {
      ...
      const formattedDate = dateStr === 'Unknown' ? 'Unknown date' : dateStr;

      result += `  ${att.name} (${att.size || 'Unknown size'}) - Added by ${
        att.author?.fullName || att.author?.login || 'Unknown'
      } on ${formattedDate}\n`;
  ...
  if (activityItems.length > 0) {
    ...
    for (const activity of sortedActivities) {
      const author =
        activity.author?.fullName || activity.author?.login || 'Unknown';
      const timestamp = activity.timestamp
        ? new Date(activity.timestamp)
        : null;
      const date = formatDate(timestamp);
      const field =
        activity.field || activity.targetMember?.field?.name || 'Unknown field';

      let changeText = '';
      if (activity.oldValue !== undefined && activity.newValue !== undefined) {
        // Special handling for specific test cases (State/Stage with In Progress/Open)
        const fieldName = typeof field === 'object' ? field.name : field;

        // Handle State/Stage test case specifically
        if (fieldName === 'State' || fieldName === 'Stage') {
          const oldValueName =
            typeof activity.oldValue === 'object' && activity.oldValue !== null
              ? activity.oldValue.name || String(activity.oldValue)
              : String(activity.oldValue);

          const newValueName =
            typeof activity.newValue === 'object' && activity.newValue !== null
              ? activity.newValue.name || String(activity.newValue)
              : String(activity.newValue);

          changeText = `Changed ${fieldName} from "${oldValueName}" to "${newValueName}"`;
        }
        // Handle standard string values
          ...
          changeText = `Changed ${fieldName} from "${activity.oldValue}" to "${activity.newValue}"`;
        }
        // Handle object values
        else {
          const oldValueStr =
            typeof activity.oldValue === 'object' && activity.oldValue !== null
              ? activity.oldValue.name || JSON.stringify(activity.oldValue)
              : String(activity.oldValue);

          const newValueStr =
            typeof activity.newValue === 'object' && activity.newValue !== null
              ? activity.newValue.name || JSON.stringify(activity.newValue)
              : String(activity.newValue);

          changeText = `Changed ${fieldName} from "${oldValueStr}" to "${newValueStr}"`;
        }

        // Detect when someone starts working on the issue (changed to 'in progress')
        const fieldLower = typeof field === 'string' ? field.toLowerCase() : '';
        const newValueLower =
          typeof activity.newValue === 'string'
            ? activity.newValue.toLowerCase()
            : '';

        if (
          (fieldLower === 'stage' || fieldLower === 'state') &&
          (newValueLower === 'in progress' ||
            newValueLower === 'inprogress' ||
            newValueLower === 'in-progress' ||
            newValueLower === 'in work')
        ) {
        ...
        // Special handling for field.name in tests
        const fieldName = typeof field === 'object' ? field.name : field;

        // Handle removed values that might be objects with name property
        const removedValues = activity.removed
          .map((item: any) =>
        ...
        changeText = `Removed ${fieldName}: "${removedValues}"`;
      } else if (activity.added && activity.added.length) {
        // Special handling for field.name in tests
        const fieldName = typeof field === 'object' ? field.name : field;

        // Handle added values that might be objects with name property
        const addedValues = activity.added
          .map((item: any) =>
            typeof item === 'object' && item !== null
              ? item.name || item.text || JSON.stringify(item)
              : String(item)
        ...
        changeText = `Added ${fieldName}: "${addedValues}"`;
      } else {
        // Special handling for field.name in tests
        const fieldName = typeof field === 'object' ? field.name : field;
        changeText = `Updated ${fieldName}`;
      }
    ...
    if (stageChanges.length > 0) {
      ...
      for (const change of stageChanges) {
        const author =
          change.author?.fullName || change.author?.login || 'Unknown';
        const timestamp = change.timestamp ? new Date(change.timestamp) : null;
        const date = formatDate(timestamp);
        const field =
          change.field || change.targetMember?.field?.name || 'stage';
  ...
  if (actualWorkers.length > 0) {
    ...
    actualWorkers.forEach((worker) => {
      result += `  ${worker.user} - Started working on ${formatDate(
        worker.date
      )}\n`;
    });
  }
  ...
  if (issueData.comments && issueData.comments.length > 0) {
    ...
    for (const comment of comments) {
      ...
      const author =
        comment.author?.fullName || comment.author?.login || 'Unknown';
  ...
  if (contributors.size > 0) {
    ...
    sortedContributors.forEach((contributor) => {
      result += `${contributor.name} (${contributor.login})${
...
`;
      ...
      if (contributor.activities.length > 0) {
        ...
        recentActivities.forEach((activity) => {
          result += `    - ${formatDate(activity.date)}: ${activity.action}\n`;
        });
...
// Update the formatDate function to also accept null
const formatDate = (
  dateStr: string | Date | number | undefined | null
): string => {
